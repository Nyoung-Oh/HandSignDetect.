...
# gesture_df.iloc[줄 인덱스, 칸 인덱스]
# :                        모든 줄
# : -1                     0번째 칸부터 마지막 칸 미만까지 리턴
# gesture_df.iloc[:,:-1] : gesture_df의 모든 줄, 마지막 칸 앞까지 리턴
# gesture_df.iloc[:,-1] : gesture_df의 모든 줄, 마지막 칸 리턴
...
# 실수가 저장돼 있기 때문에 배열의 타입을 8바이트 실수 (float64)
...
# cv2.ml.KNearnest_create() : KNN 객체 생성
...
# knn.train(angle_arr => 거리를 계산할 keypoint 각도, cv2.ml.ROW_SAMPLE, label => 각도의 손모양)
...
with mp_hands.Hands() as hands:
    ...
    while cap.isOpened() == True: #웹캠이 정상적으로 동작하는 동안 반복
        ...
        # results.multi_hand_landmakrs : 웹캠 화면에서 손이 존재하면
        if results.multi_hand_landmarks != None:

            # 웹캠 이미지에 글자 합성
            # cv2.putText(
            #     image,                             # 웹캠 이미지
            #     text="Detect Hand!!",              # 합성 할 글자
            ...
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # hand_landmarks.lanmark : 손의 keypoint 좌표 리턴
                # j : keypoint의 index
                # lm : keypoint 좌표
                ...
                # hand_landmarks.landmark[0].x : 손위치 x좌표 (0~1)
                print("hand_lanmarks.landmark[0].x=", hand_landmarks.landmark[0].x)
                # hand_landmarks.landmark[0].y : 손위치 y좌표 (0~1)
                print("hand_lanmarks.landmark[0].y=", hand_landmarks.landmark[0].y)
                ...
                if 0 <= idx <= 10:
                    cv2.putText(image,
                                ...
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX, # 글자색
                                fontScale=1,                       # 글자크기
                                color=(0, 0, 255),                 # 글자색
                                thickness=2                        # 글자두께
                                )
                ...
                # hand_landmarks.landmarks[12]의 y 좌표
                # hand_landmarks.landmarks[9].y의 y 좌표
        ...
        if cv2.waitKey(1) == ord('q'): # 키보드 입력이 q이면
            ...
            # image 변수에 저장된 웹캠 이미지를 cam_img.jgp 파일로 저장
