...
# exist_ok=True : hand_dataset 디렉토리가 이미 존재한다면 디렉토리를 만들지 않음
...
with mp_hands.Hands() as hands:
    ...
    # cap.isOpened() : 웹캠이 정상적으로 동작하면 True, 아니면 False 리턴
    while cap.isOpened()==True: # 웹캠이 정상적으로 동작하는 동안 반복

        # 프레임 1 증가
        frame = frame + 1

        # action 별로 600 프레임씩 데이터 저장
        # MAX_FRAME은 600
        if frame >= MAX_FRAME:
            break
        ...
        # cv2.flip(image, 1) : 웹캠 이미지 좌우반전
        image = cv2.flip(image, 1) # 1은 좌우반전, 0은 상하반전
        ...
        # results.multi_hands_landmarks : 웹캠 화면에서 손이 없으면 None 리턴
        # results.multi_hands_landmarks != None : 화면에 손이 존재하면
        if results.multi_hand_landmarks != None:
            ...
            # for hand_landmarks in results.multi_hand_landmarks : 탐지한 손의 keypoint 순서대로 1개식 hand_landmarks에 저장하고 반복문 실행
        ...
        # cv2.imshow(윈도우 타이틀, 윈도우창에 출력할 이미지) : 웹캠 화면을 화면에 출력
...
# 손의 좌표와 각도가 저장된 all_data를 hand_dataset 디렉토리에 action명으로 저장
