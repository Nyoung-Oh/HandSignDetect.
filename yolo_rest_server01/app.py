...
# 학습 결과 sign_language_yolo_model.pt를 이용해서 yolo 모델 생성
model = YOLO('c:/ai_project01/sign_language_yolo_model.pt')

app = Flask(__name__)

# post 방식의 hello_rest_server url을 입력하면 함수가 호출되는 rest server의 메소드
@app.route('/hello_rest_server', methods=['POST'])
def hello_world():
    return '안녕 난 rest server야'

# post 방식의 param_rest_server url을 입력하면 함수가 호출되는 rest server의 메소드
@app.route("/param_rest_server", methods=['POST'])
def hello_rest2():
    #Rest Server로 전송한 name 파라미터의 앖을 꺼내서 param_name 변수에 저장
    param_name = request.form.get('name', '입력값 없음')
    print("param_name = ", param_name)
    #리턴값
    return "그래 너의 이름은 " + param_name + "이구나!!"

# post 방식의 image_test02 url을 입력하면 함수가 호출되는 rest server의 메소드
@app.route("/image_test02", methods=["POST"])
def image_send_test02():
    # request.get_json() : 스프링 컨트롤러에서 보낼 웹 캠 이미지를 리턴
    ...
    with open('image.jpg', 'wb') as f:
        # decoded_data : 스프링 컨트롤러로부터 전송 받은 문자열을 이미지로 변환, 파일로 저장
        f.write(decoded_data)

    return "스프링이 보낸 이미지를 잘 저장했습니다!!";
...
def image_send_test01():
    ...
    print("image = ", image)
    # 스프링 컨트롤러로 메시지 전송
    return "스프링이 보낸 이미지를 잘 받았습니다!!";


# post 방식의 detect url을 입력하면 함수가 호출되는 rest server의 메소드
...
def detect_yolo():
    ...
    # cv2.imdecode(nparr, cv2.IMREAD_COLOR) : 1차원 배열 nparr을 RGB형태의 3차원 배열로 변환
